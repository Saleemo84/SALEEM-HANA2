...
const Backup: React.FC<BackupProps> = ({ accounts, onAddAccount, onRemoveAccount, showToast }) => {
    const [isModalOpen, setModalOpen] = useState(false);
    
    const handleAddAccount = (email: string) => {
        onAddAccount(email);
        showToast(`Google account ${email} connected successfully for backups!`);
    };

    const handleRemoveAccount = (id: string, email: string) => {
        onRemoveAccount(id);
        showToast(`Google account ${email} disconnected.`);
    };

    const handleBackupNow = () => {
        if (accounts.length === 0) {
            showToast('Please connect a Google account first to create a backup.', 'error');
            return;
        }
        showToast(`Creating backup... This may take a moment.`, 'success');
            ...
            <div className="bg-white rounded-lg shadow-sm border border-slate-200/80 p-4 md:p-6 animate-fadeIn space-y-6">
                <div>
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-slate-200 pb-4 mb-6 gap-4">
                        <h2 className="text-xl font-semibold text-slate-800">Google Account Backup & Storage</h2>
                        <button onClick={() => setModalOpen(true)} className="flex items-center gap-2 px-4 py-2 bg-primary text-white text-sm font-medium rounded-lg hover:bg-primary-dark transition-colors shadow-sm">
                            <i className="fab fa-google"></i> Add Google Account
                        </button>
                    </div>
                    ...
                    {accounts.length > 0 ? (
                            ...
                            {accounts.map(account => (
                                <div key={account.id} className="bg-slate-50 p-5 rounded-lg border border-slate-200 shadow-sm transition-shadow hover:shadow-md">
                                    <div className="flex items-center gap-4 mb-3">
                                        <i className="fab fa-google-drive text-3xl text-green-600"></i>
                                        <div>
                                            <p className="font-semibold text-slate-800 truncate">{account.email}</p>
                                            <p className="text-xs text-slate-500">
                                                Last Backup: {account.backupAt ? new Date(account.backupAt).toLocaleString() : 'Never'}
                                            </p>
                                        </div>
                                    </div>
                                    <button
                        ...
                        <div className="text-center py-12">
                            <i className="fab fa-google-drive text-5xl text-slate-300 mb-4"></i>
                            <p className="text-slate-500">No Google accounts connected.</p>
                            <p className="text-sm text-slate-400 mt-2">Add an account to back up your clinic data.</p>
                        </div>
                    )}
                </div>
                
                <div className="bg-slate-50 rounded-lg p-6 border border-slate-200">
                     <h3 className="text-lg font-semibold text-slate-800 mb-3">Backup Actions</h3>
                     <p className="text-sm text-slate-600 mb-4">Secure your clinic's data by creating a backup to your connected Google Drive account. You can restore from a previous backup if needed.</p>
                     <div className="flex flex-col sm:flex-row gap-4">
                         <button onClick={handleBackupNow} className="flex-1 flex justify-center items-center gap-2 px-4 py-3 bg-success text-white font-medium rounded-lg hover:bg-green-600 transition-colors shadow-sm">
                            <i className="fas fa-cloud-upload-alt"></i> Backup Now
                        </button>
                        <button onClick={() => showToast('Restore functionality coming soon!')} className="flex-1 flex justify-center items-center gap-2 px-4 py-3 bg-accent text-white font-medium rounded-lg hover:bg-accent-dark transition-colors shadow-sm">
                            <i className="fas fa-history"></i> Restore from Backup
                        </button>
                     </div>
                </div>
